...
// Получение данных для письма
		if ($value != '' &&
			...
			$key != 'city'){
				...
				<td style='padding: 10px; border: #e9e9e9 1px 
				solid;'><b>$key</b></td>
				<td style='padding: 10px; border: #e9e9e9 1px 
				solid;'>$value</td>
				</tr>
...
// Настройки PHPMailer
	...
	try {
		...
		$mail->Username = 'ЛОГИН'; // Логин на почте
		$mail->Password = 'ПАРОЛЬ'; // Пароль для внешнего приложения
		$mail->SMTPSecure = 'ssl';
		$mail->Port = 465;
		$mail->setFrom('mf-slon@mfslon.ru', 'Лид с лендинга mf-slon.ru'); // Адрес самой почты и имя отправителя
// Получатели письма
...
// Добавление нового лида в CRM Bitrix
	Add_new_lead($lead_info['CITY'], $lead_info['THEME'], 
	$lead_info['NAME'], $lead_info['TEL'], $lead_info['COMMENT'], 
	!empty($file['name'][0]));
}
